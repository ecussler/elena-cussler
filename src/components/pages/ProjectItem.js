...
function ProjectItem(props) {
  ...
  return (
    <div id='project-container' className="flex flex-wrap w-full grid-flow-row grid-cols-3 items-top gap-4 overflow-y-hidden overflow-x-scroll px-10 pt-1 pb-10 xl:grid xl:overflow-x-auto xl:px-4">
    {props.projects.map(item => (
        <div className="card bg-base-100 shadow-xl m-2 w-96 ">
         <figure><img className="items-center py-3"src={item.thumb}></img></figure>
        <div className="card-body">
            <h2 className="card-title">{item.name}</h2>
            
            {/* <div className="badge badge-accent">Techs</div> */}
            <p>{item.description}</p>
            <p className="py-2">My role: {item.role}</p>
